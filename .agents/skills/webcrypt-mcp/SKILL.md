---
name: webcrypt-mcp
description: Teaches the agent to use the WebCrypt MCP server for AES-256-GCM symmetric encryption, RSA-4096 hybrid encryption, key generation, digital signatures, hashing, and post-quantum cryptography.
---
...
| `manage_keys` | `action` ('generate' \| 'generate_random_password'), `type` ('rsa' \| 'ecdh' \| 'hmac'), `modulusLength`?, `namedCurve`?, `length`? | Generate cryptographic keys (JWK format) or secure random passwords. |
